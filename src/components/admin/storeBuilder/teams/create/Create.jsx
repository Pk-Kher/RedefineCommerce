...
const Create = () => {
  ...
  const validationSchema = Yup.object({
    name: Yup.string().required(ValidationMsgs.team.nameRequired),
    storeid: Yup.string().required(ValidationMsgs.team.storeIdRequired),
    recstatus: Yup.string().required(ValidationMsgs.common.recStatusRequired),
  });

  function onSubmit(fields, { resetForm }) {
    if (isAddMode) {
      createTeam(fields, resetForm);
    } else {
      updateTeam(fields, resetForm);
    }
  }

  function createTeam(fields, resetForm) {
    TeamService.createTeam({ ...fields })
      .then((response) => {
  ...
  function updateTeam(fields) {
    TeamService.updateTeam({ ...fields })
      .then((response) => {
        if (response.data.success) {
          dispatch(
            setAlertMessage({
  ...
  return (
    <>
      <title>{isAddMode === true ? "Add Team" : "Edit Team"}</title>
      <Formik
                ...
                <div className="flex mb-8 justify-between">
                  <div className="flex items-center">
                    ...
                    <h1 className="text-2xl md:text-3xl text-gray-800 font-bold">
                      {isAddMode === true ? "Add Team" : "Edit Team"}
                    </h1>
                  </div>
