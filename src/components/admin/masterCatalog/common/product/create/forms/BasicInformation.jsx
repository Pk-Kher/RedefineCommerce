...
const BasicInformation = ({
  ...
  const API = (type == productType.MC ? MasterProductService : type == productType.GMC ? GrandMasterProductService : [productType.EcommerceStore, productType.CorporateStore].includes(type) ? StoreProductService : BundleProductService);
  const categoryDropDownAPI = (![productType.EcommerceStore, productType.CorporateStore].includes(type) ? CategoryService.getCategoryDropdownOptions : CategoryService.getStoreCategoryDropdownOptions);
  ...
  useEffect(() => {
    var tableName = [productType.EcommerceStore, productType.CorporateStore].includes(type) ? "storebrand" : "brand"
    var storeID = [productType.EcommerceStore, productType.CorporateStore].includes(type) ? storeId : 0

    getCompanionList()

    DropdownService.getDropdownValues(
      tableName, false, storeID
    ).then((response) => {
      setBrandList(() => {
  ...
  const VendorDropDownAPI = ![productType.EcommerceStore, productType.CorporateStore].includes(type) ? VendorService.getVendorByBrand : VendorService.getStoreVendorByBrand;
  ...
  const submitHandler = (fields, { resetForm }) => {
    ...
    let moduleName = (type == productType.MC ? "MasterCatalog" : type == productType.GMC ? "GrandMasterCatalog" : [productType.EcommerceStore, productType.CorporateStore].includes(type) ? "Store" : "Bundle");
        ...
        .then((response) => {
          if (response.data.success) {
            ...
            if (moduleName === "Bundle") {
              navigate(`/admin/MasterCatalog/${storeType}/${storeName}/${storeId}/products/${moduleName}/edit/${response.data.data.id}`)
            } else {
              navigate(`/admin/MasterCatalog/${moduleName}/edit/${response.data.data.id}`)
            }
          } else {
            dispatch(
            ...
            <>
              <FormikForm>
                ...
                <div className="panel-01 tab-content p-6">
                    ...
                    <>
                      <div className="mb-6 last:mb-0 pt-6">
                        ...
                        <Dropdown
                          ...
                          isDisabled={readOnly}
                          onChange={(data) => {
                            setFieldValue('brandId', (data ? Number(data.value) : ''));
                            if (data && data?.value == basicInfo?.brandId) {
                              setFieldValue('vendorId', basicInfo?.vendorId);
                            } else if (values?.vendorId) {
                              setFieldValue('vendorId', '');
                            }
                            if (data?.value) {
                              getVendorOption(data?.value);
                            } else {
                              setVendorList([]);
                            }
                          }}
                  ...
                  {/* Different ERP Name checkbo display along with Textbox for ERP Name if different than product name */}
                          ...
                          <>
                            <div className="w-full mb-6 last:mb-0">
                              ...
                              <InputNumber
                                ...
                                decimalScale={0}
                                onChange={(e) => {
                                  setFieldValue('erpItemId', Number(e.target.value));
                                }}
                                displayError={true}
                              />
                            </div>
                      ...
                      <>
                        <div className="mb-6">
                          ...
                          <Input
                            ...
                            maxLength={100}
                            id="vendorSKU"
                            className={`block w-full border border-neutral-200 hover:border-neutral-300 focus:border-neutral-300 focus:ring-0 focus:shadow-lg px-2 py-2 rounded-md`}
                            disabled={readOnly}
                          />
                        </div>
                      </>
                      ...
                      <>
                        <div className="mb-6 mt-auto">
                          ...
                          <CustomSearchDropDown
                            ...
                            onChange={(value) => {
                              setFieldValue('companionProduct', (value ? value : ""))
                            }}
                            options={SKUList}
                      ...
                      <div className="w-full mb-6 last:mb-0">
                        ...
                        <Dropdown
                          ...
                          onChange={(value, event) => {
                            setFieldValue('dimensionTemplateId', (value ? Number(value.value) : 0));
                            handleDimensionChange(value, { ...values, dimensionTemplateId: Number(value.value) }, setValues);
                          }}
                          isDisabled={readOnly}
                          isClearable={true}
                        />
