...
const CustomerFAQ = ({ values, readOnly, isAddMode, type, productId, getProductReadinessData }) => {
  const API = ([productType.EcommerceStore, productType.CorporateStore].includes(type) ? StoreCustomerFaqsService : BundleCustomerFaqsService)
      ...
      <div className="w-full">
        <div className="max-h-screen border-t border-neutral-200">
          <ReactTable
            ...
            extraFilter={readOnly ? [] : [{ Component: AddButton, isAddMode, type, productId, getCustomerFaqsData, API, getProductReadinessData }]}
...
const AddButton = ({ isAddMode, type, productId, getCustomerFaqsData, API, getProductReadinessData }) => {
...
const AddFaqModal = ({ openAddModal, setopenAddModal, getCustomerFaqsData, productId, type, API, getProductReadinessData }) => {
  const dispatch = useDispatch();
  const location = useSelector((store) => store?.location);
