...
const Personalization = ({
  ...
  const COLUMNS = [
    ...
    },
    {
      ...
      Cell: ({ value }) => {
        return value ? (
          <>
            {value.brandGuidelines && <div >{`${value.brandGuidelines}`}</div>}
          </>
        ) : (
          "-"
        );
  ...
  const enableLogoLocationHandler = (e) => {
    let ourCurrentModuleName = (type == productType.MC ? MasterProductService : StoreProductService);
    const ourReqObj = [
      {
  ...
  return (
    <>
                ...
                <div className="w-full">
                  ...
                  <div className="flex flex-wrap p-6 pb-5 pt-5 gap-4 items-center">
                    ...
                    <div className="flex items-center">
                      <ToggleButton
                        onChange={(e) => {
                          setFieldValue('isEnableLogolocation', e.target.checked);
                          enableLogoLocationHandler(e);
                        }}
      ...
      <BasicModal
        handleConfirmation={submitHandler}
