...
import AddMessageModel from "../create/AddMessageModel";
...
const List = () => {
  ...
  const handleDelete = (message) => {
    var ids = [];
    if (message.length > 0) {
      ids = message.map((value) => {
        return { item1: value.id, item2: value.rowVersion };
      });
    } else {
      ids = [{ item1: message.id, item2: message.rowVersion }];
    }
    dispatch(setAddLoading(true));

    MessageServices.updateMultipleStatus({
      args: {
        idsRowVersion: ids,
      ...
      .then((response) => {
        if (response.data.data) {
          dispatch(
            setAlertMessage({
              type: "success",
              message: ValidationMsgs.message.messageDeleted,
            })
          );
          getMessage();
        } else {
      ...
      .catch((errors) => {
        if (errors.response.data.Errors.Error) {
          ...
          setAlertMessage({
            message: ValidationMsgs.message.messageNotDeleted,
            type: "danger",
          });
        }
        dispatch(setAddLoading(false));
          ...
          if (response.data.data) {
            dispatch(
              setAlertMessage({
                ...
                type: "success",
                message:
                  ValidationMsgs.message.messageStatusUpdated,
              })
            );
            getMessage();
          } else {
        ...
        .catch((errors) => {
          if (errors.response.data.Errors.Error) {
            ...
            dispatch(
              setAlertMessage({
                message:
                  ValidationMsgs.message.messageStatusNotUpdated,
                type: "danger",
              })
            );
          }
  ...
  const getStoreAndMessageKeyMoreFilterData = useCallback(() => {
  ...
  useEffect(() => {
    getStoreAndMessageKeyMoreFilterData();
  }, [getStoreAndMessageKeyMoreFilterData]);
  ...
  const COLUMNS = [
    ...
    },
    {
      id: "messageKeyName",
      Header: "Message Key",
      accessor: "messageKeyName",
      column_name: "messageKeyName",
    },
    {
      id: "storeName",
      Header: "Store",
      ...
      <div className="px-4 sm:px-6 lg:px-8 py-8 w-full">
        ...
        <div className="col-span-full w-full bg-white shadow-xxl rounded-md mb-8 relative">
          <ReactTable
            ...
            saveFilter={{ show: true, tabName: pathname + '_' + 'message' }}
          />
        </div>
      </div>
      <ConfirmDelete
        ...
        data={message}
        message={ValidationMsgs.message.messagePermanentDelete}
        title={"Delete"}
        module={"Message"}
