...
const AddMessageKeyModel = ({
    ...
    function createMessageKey(fields, resetForm) {
        dispatch(setAddLoading(true));
        MessageKeyServices.createMessageKey({
            storeMessageModel: {
                name: fields.name,
            ...
            .then((response) => {
                if (response.data.success) {
                    dispatch(
                        setAlertMessage({
                            type: "success",
                            message: ValidationMsgs.messageKey.messageKeyCreated,
                        })
                    );
                    resetForm({});
            ...
            .catch((errors) => {
                dispatch(
                    setAlertMessage({
                        type: "danger",
                        message: ValidationMsgs.messageKey.messageKeyNotCreated,
                    })
                );
                dispatch(setAddLoading(false));
            });
    ...
    function updateMessageKey(fields, resetForm) {
        dispatch(setAddLoading(true));

        MessageKeyServices.updateMessageKey({
            ...
            .then((response) => {
                if (response.data.success) {
                    dispatch(
                        setAlertMessage({
                            type: "success",
                            message: ValidationMsgs.messageKey.messageKeyUpdated,
                        })
                    );
                    handleShowModal((prev) => !prev);
                    resetForm({});
            ...
            .catch((errors) => {
                dispatch(
                    setAlertMessage({
                        type: "danger",
                        message: ValidationMsgs.messageKey.messageKeyNotUpdated,
                    })
                );
                dispatch(setAddLoading(false));
            });
    ...
    useEffect(() => {
        if (idson) {
            dispatch(setAddLoading(true));
            MessageKeyServices.getMessageKeyById(idson)
                .then((res) => {
    ...
    const onsubmit = (fields, { resetForm }) => {
        if (idson) {
            updateMessageKey(fields, resetForm);
        } else {
            createMessageKey(fields, resetForm);
        }
    };

    const validationSchema = Yup.object({
        name: Yup.string().required(ValidationMsgs.messageKey.messageKeyRequired),
    });

    return (
        <>
            <title> Add Message Key </title>
            <div
                id="messageKeyModel"
...
export default AddMessageKeyModel;
