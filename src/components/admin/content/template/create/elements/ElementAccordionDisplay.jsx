...
const ElementAccordionDisplay = ({acValues}) => {
    ...
    const iconArr = {"keyboard_arrow_up": "keyboard_arrow_down", "keyboard_arrow_down": "keyboard_arrow_up", "remove_circle_outline": "add_circle_outline", "add_circle_outline": "remove_circle_outline"};
    ...
    const showHideAccordion = (event) => {
        ...
        if(event.target.querySelector(".pointer-class"))
        {
            ...
            if(event.currentTarget.querySelector(".ac-description").classList.contains("hidden"))
                event.currentTarget.querySelector(".ac-description").classList.remove("hidden");
            else
                event.currentTarget.querySelector(".ac-description").classList.add("hidden");
        }
    }
                        ...
                        <li className='mb-4 last:mb-0 hasarr clonnable border-b border-black' onClick={showHideAccordion} >
                            <button className='w-full flex justify-between items-center text-left font-bold font-heading px-2 py-4 border-0 hover:border-0'>
                                ...
                                {acValue.openstatus == "Yes" ? <>
                                    {acValue.icon == "caret" ? "keyboard_arrow_down" : "remove_circle_outline"}
                                    </>
                                    : 
                                    <>
                                    {acValue.icon == "caret" ? "keyboard_arrow_up" : "add_circle_outline"}
                                    </>
                                    }
                                    </span>
                            </button>
                            <div className={`text-defaule-text px-2 mt-2 pb-4 ac-description ${acValue.openstatus != "Yes" ? "hidden" : ""}`}>
                                <div className="text-defaule-text mt-2" dangerouslySetInnerHTML={{ __html: acValue.desc }}></div>
                            </div>
                        </li>
                    )
                })}
