...
const SEO = ({
  ...
  // const API = ([productType.EcommerceStore, productType.CorporateStore].includes(type) ? StoreProductService : BundleProductService)
  const regexCode = /(<([^>]+)>)/gi;
        ...
        <div className="w-full">
          ...
          >
            {({ errors, touched, setFieldValue, values }) => {
              ...
              return (
                <FormikForm>
                  {!loading && <ErrorHandler setIsError={setIsError} errors={errors} checkProductStatus={checkProductStatus} />}
                          ...
                          <div className="mb-6">
                            <div className={`mb-3 text-base bg-white py-4 rounded ${values.previewType === 'D' ? 'w-full' : ' w-2/4'}`}>
                              <div className="text-sm leading-4 text-[#202124] flex py-1 font-arial">
                                <a href={`${store ? store?.url : ''}/${values.seName}`} target="_blank" className="cursor-pointer" title={`${store ? store?.url : ''}/${values.seName}`}>{store ? store?.url : ''} › ... <span className="material-icons-outlined text-[15px]">more_vert</span></a>
                              </div>
                        ...
                        {displayFieldElement(fields, "seName") && (
                          <div className="mb-6">
                            ...
                            <div className="flex items-center">
                              ...
                              <div className="grow">
                                <Input name={'seName'} type="text" placeholder="SE Name" className="appearance-none block w-full text-gray-700 border border-gray-200 rounded p-4 py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white" disabled={readOnly} />
                              </div>
                            </div>
                          </div>)}
                        {displayFieldElement(fields, "pageTitle") && (
