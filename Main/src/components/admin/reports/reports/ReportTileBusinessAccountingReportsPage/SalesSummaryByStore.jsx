...
const SalesSummaryByStore = () => {
  ...
  const getSalesSummaryReceivedOrdersByStore = useCallback(
    (pageIndex) => {
      if (store?.value) {
        dispatch(setAddLoading(true));
        BusinessAccountingService.getSalesSummaryReceivedOrdersByStore({
          args: {
  ...
  const Export = (pageIndex) => {
    dispatch(setAddLoading(true));
    BusinessAccountingService.ExportSalesSummaryReceivedOrdersByStore({
      args: {
        pageIndex: 0,
        pageSize: paginationData.pageSize,
        pagingStrategy: 0,
  ...
  useEffect(() => {
    getSalesSummaryReceivedOrdersByStore();
  }, [store?.value]);
  ...
  const COLUMNS = [
    ...
    {
      id: "coupons",
      Header: `coupons (${CurrencySymbolByCode.USD})`,
      accessor: "coupons",
      column_name: "coupons",
      Cell: ({ value }) => {
        return value ? (
      ...
      Footer: () => {
        return (
          <>
            {CurrencySymbolByCode.USD}
            {Data?.total?.totalCoupons.toFixed(2)}
          </>
        );
      },
    },
    ...
    {
      id: "giftWrap",
      Header: `GIFT WRAP (${CurrencySymbolByCode.USD})`,
      accessor: "giftWrap",
      column_name: "giftWrap",
      Cell: ({ value }) => {
        return value ? (
          <>
            <div className="text-left">{value.toFixed(2)}</div>
      ...
      Footer: () => {
        return (
          <>
            {CurrencySymbolByCode.USD}
            {Data?.total?.totalGiftWrap.toFixed(2)}
          </>
        );
      },
    },
    {
      id: "adjustment",
      Header: `ADJUSTMENT (${CurrencySymbolByCode.USD})`,
      accessor: "adjustment",
      ...
      Footer: () => {
        return (
          <>
            {CurrencySymbolByCode.USD}
            {Data?.total?.totalAdjustment.toFixed(2)}
          </>
        );
      },
    },
  ...
  return (
    <>
      <title>Sales summary By Store (Received Orders)</title>
        ...
        <div className="bg-white shadow-xxl rounded-md mb-8">
          ...
          <div className="overflow-x-auto max-h-screen">
            <ReactTable
              ...
              fetchData={getSalesSummaryReceivedOrdersByStore}
...
export default SalesSummaryByStore;
