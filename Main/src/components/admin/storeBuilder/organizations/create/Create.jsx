...
const Create = () => {
  ...
  const validationSchema = Yup.object({
    name: Yup.string().required(ValidationMsgs.organization.nameRequired),
    storeid: Yup.string().required(ValidationMsgs.organization.storeIdRequired),
    recstatus: Yup.string().required(ValidationMsgs.common.recStatusRequired),
  });

  function onSubmit(fields, { resetForm }) {
    if (isAddMode) {
      createOrganization(fields, resetForm);
    } else {
      updateOrganization(fields, resetForm);
    }
  }

  function createOrganization(fields, resetForm) {
    OrganizationService.createOrganization({ ...fields })
      .then((response) => {
  ...
  function updateOrganization(fields) {
    OrganizationService.updateOrganization({ ...fields })
      .then((response) => {
        if (response.data.success) {
          dispatch(
            setAlertMessage({
  ...
  return (
    <>
      <title>{isAddMode === true ? "Add Organization" : "Edit Organization"}</title>
      <Formik
              ...
              <div className="px-4 sm:px-6 lg:px-8 py-8 w-full">
                <div className="flex mb-8 justify-between">
                  <div className="flex items-center">
                    <Link
                      to={"/admin/StoreBuilder/organizations"}
                    ...
                    <h1 className="text-2xl md:text-3xl text-gray-800 font-bold">
                      {isAddMode === true ? "Add Organization" : "Edit Organization"}
                    </h1>
                  </div>
